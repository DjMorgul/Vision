...
WindowList::AddAgent(BView* _agent, const char* name, int32 winType, bool activate)
{
	...
	if (dynamic_cast<ServerAgent*>(agent) != NULL)
		...
		if (dynamic_cast<ClientAgent*>(agent) != NULL)
			agentParent = dynamic_cast<ServerAgent*>(
				dynamic_cast<ClientAgent*>(agent)->fSMsgr.Target(&looper));
		else
			agentParent = dynamic_cast<ServerAgent*>(
				dynamic_cast<ListAgent*>(agent)->fSMsgr->Target(&looper));
...
WindowListItem::DrawItem(BView* father, BRect frame, bool complete)
{
	...
	father->MovePenTo(frame.left + 4, frame.bottom - fh.descent);
