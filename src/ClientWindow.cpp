...
void
DynamicEditMenu::DetachedFromWindow()
{
...
{
	switch (msg->what) {
		// this is somewhat annoying but these are needed in addition to the ones in the input
		// filter since the menu items can't send the keyboard accelerators and only send the
		// message ; annoying code duplication but oh well
		case M_UP_CLIENT:
		{
		...
		case M_CW_UPDATE_STATUS:
		{
			...
			if ((msg->FindPointer("item", reinterpret_cast<void**>(&item)) != B_OK)
				...
				printf(
					":ERROR: no valid pointer and int found in M_CW_UPDATE_STATUS, bailing...\n");
				return;
			}
		...
		case M_MAKE_NEW_NETWORK:
		{
			...
			pWindowList()->AddAgent(new ServerAgent(netName.String(), network), netName.String(),
				WIN_SERVER_TYPE,
		...
		case M_OPEN_TERM:
		{
			...
			if (result != B_OK) {
				...
				ClientAgent::PackDisplay(
					&errMsg, errString.String(), C_ERROR, C_BACKGROUND, F_TEXT);
...
WindowList*
ClientWindow::pWindowList() const
{
	return fCwDock->pWindowList();
}

NotifyList*
ClientWindow::pNotifyList() const
{
	return fCwDock->pNotifyList();
}

ClientWindowDock*
ClientWindow::pCwDock() const
{
	return fCwDock;
}

StatusView*
ClientWindow::pStatusView() const
{
	return fStatus;
}
...
ClientWindow::Init()
{
	...
	menu->AddItem(item = new BMenuItem(B_TRANSLATE("Help"), new BMessage(M_HELP_SHOW)));
	item->SetTarget(vision_app);
	...
	menu->AddItem(item = new BMenuItem(
					  B_TRANSLATE("Open Terminal"), new BMessage(M_OPEN_TERM), 'T', B_OPTION_KEY));
	menu->AddSeparatorItem();
	...
	fServer->AddItem(menu = new NetworkMenu(B_TRANSLATE("Connect to" B_UTF8_ELLIPSIS),
						 M_CONNECT_NETWORK, BMessenger(vision_app)));
	...
	fServer->AddItem(item = new BMenuItem(B_TRANSLATE("List channels" B_UTF8_ELLIPSIS),
	...
	fEdit->AddItem(item = new BMenuItem(
					   B_TRANSLATE("Select all"), new BMessage(B_SELECT_ALL), 'A', B_OPTION_KEY));
	...
	fWindow->AddItem(
		item = new BMenuItem(B_TRANSLATE("Up"), new BMessage(M_UP_CLIENT), B_UP_ARROW));
	fWindow->AddItem(
	...
	fWindow->AddItem(item = new BMenuItem(B_TRANSLATE("Smart down"),
						 new BMessage(M_SMART_DOWN_CLIENT), B_DOWN_ARROW, B_SHIFT_KEY));
	fWindow->AddItem(
		item = new BMenuItem(B_TRANSLATE("Network window"), new BMessage(M_NETWORK_CLIENT), '/'));
	fWindow->AddItem(item = new BMenuItem(B_TRANSLATE("Previous window"),
						 new BMessage(M_PREVIOUS_CLIENT), '0', B_SHIFT_KEY));
	AddShortcut(B_INSERT, B_SHIFT_KEY, new BMessage(M_PREVIOUS_CLIENT));
	fWindow->AddSeparatorItem();
	fWindow->AddItem(item = new BMenuItem(B_TRANSLATE("Move network up"),
