	...
	switch (msg->what) {
		...
		case B_MIME_TYPE:
		{
			if (msg->HasData("text/plain", B_MIME_TYPE)) {
				...
				msg->FindData(
					"text/plain", B_MIME_TYPE, 0, reinterpret_cast<const void**>(&buffer), &size);
	...
	if (lines > 1) {
		if (true == vision_app->GetBool("Newbie Spam Mode")) {
			...
			str.SetToFormat(B_TRANSLATE("As if there isn't enough, you are "
										...
										"internet.  How would you like to go about this?"),
				lines);

			BAlert* alert(new BAlert(B_TRANSLATE("Spam"), str.String(), B_TRANSLATE("Cancel"),
				B_TRANSLATE("Spam!"), B_TRANSLATE("Single line"), B_WIDTH_FROM_WIDEST,
